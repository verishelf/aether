...
export function CollectionPanel({ assets }: { assets: CollectionAsset[] }) {
  ...
  return <article className="collection-panel"><div className="rightbar-heading"><span className="eyebrow">My collection</span><Link href="/app/collection" aria-label="Manage collection"><ArrowUpRight size={16} /></Link></div>{items.length ? <div className="collection-list">{items.slice(0, 5).map((asset) => <div className="collection-row" key={asset.id}>{asset.image_paths[0] ? <div className="collection-thumb" style={{ backgroundImage: `url(${asset.image_paths[0]})` }} /> : <div className="collection-thumb collection-thumb-empty" /> }<span><strong>{asset.title}</strong><small>{asset.category}</small></span><button type="button" onClick={() => remove(asset.id)} aria-label={`Delete ${asset.title}`}><Trash2 size={13} /></button></div>)}</div> : <p className="collection-empty">Your collection is waiting for its first piece.</p>}<Link className="collection-link" href="/app/collection">Add, edit, or manage assets <span>↗</span></Link>{error && <p className="auth-error" role="alert">{error}</p>}</article>;
}
