...
export default function Home() {
  ...
  return (
    <main className="site-shell">
      ...
      <section className="membership section-grid" id="membership"><p className="eyebrow">04 / Membership</p><div className="membership-content"><div><h2 className="section-title">Your membership<br /><em>awaits.</em></h2><p className="large-copy">Choose your cadence and continue directly to secure Stripe checkout.</p></div><div className="pricing-box"><div className="billing-toggle" role="group" aria-label="Billing frequency"><button className={!annual ? "active" : ""} onClick={() => setAnnual(false)}>Monthly</button><button className={annual ? "active" : ""} onClick={() => setAnnual(true)}>Yearly <span>Save 17%</span></button></div><div className="price"><span>$</span><strong>{annual ? "247" : "297"}</strong><small>/ month</small></div><p className="price-note">{annual ? "Billed annually at $2,970" : "Billed monthly"}</p><MembershipCheckout plan={annual ? "yearly" : "monthly"} /><ul><li><Check size={15} /> Full access to the network</li><li><Check size={15} /> All private circles</li><li><Check size={15} /> Direct messaging & discovery</li></ul></div></div></section>
